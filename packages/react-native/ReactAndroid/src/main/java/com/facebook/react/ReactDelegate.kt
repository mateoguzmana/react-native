...
public open class ReactDelegate {
   ...
   */
  protected var isFabricEnabled: Boolean = ReactNativeNewArchitectureFeatureFlags.enableFabricRenderer()
    private set

  /**
   * Do not use this constructor as it's not accounting for New Architecture at all. You should use
   * [ReactDelegate(Activity, ReactNativeHost, String, Bundle, boolean)] as it's
   * the constructor used for New Architecture.
   */
  @Deprecated("Use one of the other constructors instead to account for New Architecture. Deprecated since 0.75.0")
  public constructor(
      activity: Activity,
  ...
  private val devSupportManager: DevSupportManager?
    get() = if (ReactNativeNewArchitectureFeatureFlags.enableBridgelessArchitecture()
        && reactHost != null && reactHost?.devSupportManager != null
    ) {
  ...
  public fun onActivityResult(
      requestCode: Int, resultCode: Int, data: Intent?, shouldForwardToReactInstance: Boolean
  ) {
    if (ReactNativeNewArchitectureFeatureFlags.enableBridgelessArchitecture()) {
  ...
  public fun onConfigurationChanged(newConfig: Configuration) {
    if (ReactNativeNewArchitectureFeatureFlags.enableBridgelessArchitecture()) {
      ...
      if (reactNativeHost?.hasInstance() == true) {
        getReactInstanceManager()
            .onConfigurationChanged(checkNotNull(activity), newConfig)
      }
    }
  }
  ...
  public fun onKeyLongPress(keyCode: Int): Boolean {
    if (keyCode == KeyEvent.KEYCODE_MEDIA_FAST_FORWARD) {
      if (ReactNativeNewArchitectureFeatureFlags.enableBridgelessArchitecture()
        ...
        if (reactNativeHost?.hasInstance() == true && reactNativeHost?.useDeveloperSupport == true) {
  ...
  public fun shouldShowDevMenuOrReload(keyCode: Int, event: KeyEvent?): Boolean {
    ...
    val didDoubleTapR =
        checkNotNull(doubleTapReloadRecognizer)
  ...
  public fun getReactInstanceManager(): ReactInstanceManager {
    return requireNotNull(reactNativeHost?.reactInstanceManager)
  }
